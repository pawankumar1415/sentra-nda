"""
rag_function/guidance_loader.py — Loads Good Practice guidance text from Azure Blob Storage.

Connects to Blob Storage using DefaultAzureCredential (managed identity in Azure,
env-var credential chain locally).  Caches the extracted text by blob etag so
warm instances never re-download unless the blob has actually changed.
...
# ── Fallback guidance (mirrors current hardcoded rules) ───────────────────────
_FALLBACK_GUIDANCE = """
...
def get_guidance_text() -> tuple[str, str]:
    ...
    container_name = os.environ.get("AZURE_STORAGE_CONTAINER_NAME", "").strip()
    ...
    try:
        ...
        # Cheap properties call to get current etag
